...
class MyMainPageState extends State<MainPage> {
  ...
  Widget build(BuildContext context) {
    ...
    _token = authUser
        .accessToken; // TODO Delete Hardcode solution when the global token will implemented

    return MultiBlocProvider(
        providers: [
          BlocProvider(
            create: (_) =>
            MyCasesCubit(repository)
              ..fetchCases(user.userProfileId, authUser.accessToken),),
          BlocProvider(
              create: (_) => FindPatientCubit(findRepository))
          // Esto me daña la cohecion de ventanas, pero es una medida desesperada
        ],
        child:
